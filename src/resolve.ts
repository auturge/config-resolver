...
import { getArgumentNullError, getRequiredConfigMissingError, getOptionsValidationError } from "./errors";
...
export function resolve(options: ResolverOptions): ResultType {

    if (!options)
        throw getArgumentNullError('options');

    if (!options.explicit && !(options.alternatives && options.alternatives.length))
